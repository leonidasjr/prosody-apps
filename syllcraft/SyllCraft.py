...
class CustomDialog(simpledialog.Dialog):
    def body(self, master):
        ...
        self.title("SyllCraft © -  Leônidas Silva Jr. (2024-2026)")
...
with open("segmented_words.txt", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f, delimiter="\t")
    # Write header
    writer.writerow(["Word", "Ortho", "Ortho_Syll", "Phon", "Phon_Syll", "SSP-MOP", "SSP-MOP_Syll"])
    # Write rows
    for row in data:
        # Convert lists (like syllables) into strings joined by hyphens
        formatted_row = [
            row[0],
            "-".join(row[1]) if isinstance(row[1], list) else row[1],
            row[2],
            "-".join(row[3]) if isinstance(row[3], list) else row[3],
            row[4],
...
print("\nResults have been saved to 'segmented_words.txt' (tab-delimited).")
